...
class QuadraticCost(TrajoptCost):
	...
	def value(self, x, u = None, timestep = None):
		delta_x = x - self.xg
	...
	def gradient(self, x, u = None, timestep = None):
		delta_x = x - self.xg
		currQ = self.get_currQ(u,timestep)
	...
	def hessian(self, x, u = None, timestep = None):
		nx = self.Q.shape[0]
		nu = self.R.shape[0]
		currQ = self.get_currQ(u,timestep)
		if u is None:
	...
	def increase_QF(self, multiplier = 2):
		self.QF *= multiplier
		self.increaseCount_QF += 1
		return self.increaseCount_QF

	def increase_Q(self, multiplier = 2):
		self.Q *= multiplier
	...
	def shift_QF_start(self, shift = -1):
		self.QF_start += shift
